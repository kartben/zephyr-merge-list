...
def merge_status(data):
    ...
    if not data.assignee:
        return ("blocked", "Needs assignee approval",
...
def gate_cell(css, symbol, hint):
    """One table cell in the merge gate columns: a tooltip plus a
    symbol, left empty when the CSS class draws an icon instead."""
...
def render_gates(data):
    ...
    if data.rebaseable is None:
        ...
        conflict = gate_cell("gate-pass", "", "No merge conflicts")
    else:
        conflict = gate_cell("gate-fail", "",
                             "Has merge conflicts: needs a rebase")

    if data.assignee:
        approval = gate_cell("gate-pass", "",
